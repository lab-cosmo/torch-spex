...
class Chebyshev(Simple):
    ...
    The Chebyshev polynomials are essentially ``cos(n*arccos(2*x/cutoff-1))``
    where ``n`` enumerates the different basis functions.
    ...
    def expand(self, r):
        ...
        y = torch.where(mask, 0, torch.cos(k * torch.arccos(2 * r - 1)))
